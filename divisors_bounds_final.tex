...
\]
We verify in turn that $\varphi$ is well-defined, injective, and surjective.

\medskip
\emph{Well-definedness.}\;
Let $d \in S_1$ be arbitrary; we must show $\varphi(d) = n/d \in S_3$.
Since $d \in S_1 \subseteq D$, $d$ divides $n$, so $n/d \in \mathbb{N}$ and
...
\medskip
\emph{Injectivity.}\;
Suppose $a, b \in S_1$ satisfy $\varphi(a) = \varphi(b)$, i.e.\ $n/a = n/b$.
Multiplying both sides by $ab/n$ gives $a = b$, so $\varphi$ is injective.

\medskip
\emph{Surjectivity.}\;
Let $y \in S_3$ be arbitrary; we must find $x \in S_1$ with $\varphi(x) = y$.
Solving $n/x = y$ gives $x = n/y$, so we set $x := n/y$. Since
$y \in S_3 \subseteq D$, $y$ divides $n$, so $x \in \mathbb{N}$ and $x$ divides
$n$. Moreover, since $y > \sqrt{n}$,
\begin{alignat}{2}
    \frac{1}{y} &< \frac{1}{\sqrt{n}}
        &\qquad& \text{(taking reciprocals of positive quantities reverses the inequality)} \notag \\
    x = \frac{n}{y} &< \frac{n}{\sqrt{n}} = \sqrt{n}
        &\qquad& \text{(multiplying both sides by $n > 0$)} \notag
\end{alignat}
so $x \in S_1$. Finally, $\varphi(x) = n\,/\,(n/y) = y$, so $x$ is indeed the
required preimage. Since $y \in S_3$ was arbitrary, $\varphi$ is surjective.

\medskip
Since $\varphi$ is a bijection from $S_1$ to $S_3$, it follows that
$|S_1| = |S_3|$.
...
% ===============================================================
\section*{An Alternative Approach for Step 1}
% ===============================================================

This section presents an alternative approach to exhibit a bijection
between $S_1$ and $S_3$.

\bigskip

\noindent We show $|S_1| = |S_3|$ via the map
\[
    \varphi \;:\; S_1 \;\longrightarrow\; S_3, \qquad \varphi(d) = \frac{n}{d}.
\]
